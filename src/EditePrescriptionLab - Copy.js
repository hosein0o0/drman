        ...
        for(let value in this.state){
            if(value.includes('labInputs')){
                ...
                if(listLab[j]===name_test){
                    ...
                    listItem['Test_name']=!this.state.Test_name?name_test:this.state.Test_name
...
class ShowResultLab extends Component{
    ...
    ChangeTestName=(env)=>{
        this.setState({
            Name_test:env.target.value
        })
        this.props.ChangeTestName(env.target.value)
    }
    handleShow=(name)=>{
        let AllData=[]
        let count=-1
    ...
    return(
            ...
            <div className='col-6'>
            <input className='w-100 p-2' value={this.state.Name_test===undefined?data.Test_name:null} onChange={this.ChangeTestName}/>
            </div>
            </div>
            <div className='col-12'>
            <label>موارد :</label>
